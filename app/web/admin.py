...
async def payments(_: AdminAuth, session: AdminSession) -> Response:
    ...
    return _layout("Платежи", f"<table><tr><th>ID</th><th>User</th><th>Place</th><th>Тариф</th><th>Status</th><th>Stars</th><th>Charge</th><th>Дата</th></tr>{rows}</table>")
...
def _payment_row(payment: AdPayment) -> str:
    ...
    <tr>
      ...
      <td>{_escape(payment.status)}</td><td>{payment.stars_amount}⭐</td><td>{_escape(payment.telegram_payment_charge_id)}</td><td>{_escape(payment.created_at)}</td>
    </tr>
    """


@router.get("/events", response_class=HTMLResponse)
async def events(_: AdminAuth) -> Response:
    service = EventService()
    city_names = ["Москва", "Санкт-Петербург", "Новосибирск", "Екатеринбург", "Казань"]
    all_events = []
    for city in city_names:
        all_events.extend(await service.list_events(city, limit=10))
    rows = "".join(_event_row(event) for event in all_events)
    return _layout(
        "События",
        "<table><tr><th>ID</th><th>Название</th><th>Город</th><th>Категория</th>"
        "<th>Площадка</th><th>Когда</th><th>Цена</th></tr>"
        f"{rows}</table>",
    )


def _event_row(event: object) -> str:
    return f"""
    <tr>
      <td>{_escape(getattr(event, "external_id", ""))}</td>
      <td>{_escape(getattr(event, "title", ""))}<br><span class="muted">{_escape(getattr(event, "description", ""))}</span></td>
      <td>{_escape(getattr(event, "city", ""))}</td><td>{_escape(getattr(event, "category", ""))}</td>
      <td>{_escape(getattr(event, "venue", ""))}</td><td>{_escape(getattr(event, "starts_at", ""))}</td>
      <td>{_escape(getattr(event, "price_label", ""))}</td>
    </tr>
    """


@router.get("/analytics", response_class=HTMLResponse)
async def analytics(_: AdminAuth, settings: Annotated[AppSettings, Depends(get_settings)]) -> Response:
    from redis.asyncio import Redis

    redis = Redis.from_url(settings.redis_url, decode_responses=True)
    try:
        event_keys = await redis.keys("analytics:event:*")
        rows = []
        for key in sorted(event_keys):
            name = str(key).split(":")[-1]
            value = int(await redis.get(key) or 0)
            rows.append(f"<tr><td>{_escape(name)}</td><td>{value}</td></tr>")
    finally:
        await redis.aclose()
    return _layout("Аналитика", f"<table><tr><th>Событие</th><th>Количество</th></tr>{''.join(rows)}</table>")


@router.get("/feedback", response_class=HTMLResponse)
async def feedback(_: AdminAuth, settings: Annotated[AppSettings, Depends(get_settings)]) -> Response:
    from redis.asyncio import Redis

    redis = Redis.from_url(settings.redis_url, decode_responses=True)
    try:
        items = await redis.lrange("feedback:items", 0, 100)
    finally:
        await redis.aclose()
    rows = "".join(_feedback_row(item) for item in items)
    return _layout("Feedback", f"<table><tr><th>User</th><th>Город</th><th>Сообщение</th></tr>{rows}</table>")


def _feedback_row(raw: str) -> str:
    user_id, city, text = (raw.split("|", 2) + ["", "", ""])[:3]
    return f"<tr><td>{_escape(user_id)}</td><td>{_escape(city)}</td><td>{_escape(text)}</td></tr>"
